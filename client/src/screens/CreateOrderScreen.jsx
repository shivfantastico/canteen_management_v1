 ...
 *
 * DATE-AWARE MEAL STATUS + BOOKED STATE
 * ──────────────────────────────────────
 * The API (POST /api/meal) returns each meal with an `isBooked` flag
 * indicating whether the current user already has an order for that
 * meal on the requested date.
 *
 * applyMealStatus(apiMeals, date) merges:
 *   1. BASE_MEAL_TYPES meta (emoji, color, bg, time)
 *   2. getMealStatus(meal, date) — cutoff logic
 *      • date === today → normal open/closed/urgent logic
 *      • date !== today → isOpen:true, isDeadline:true (all open)
 *   3. meal.isBooked from the API response (preserved verbatim)
 *
 * MealTypePicker then uses BOTH flags:
 *   • isBooked  → disabled, shows "✔ BOOKED" badge (grey), regardless of date
 *   • !isOpen   → disabled, shows "✕ CLOSED" (today only, never future dates)
 *   • isOpen && !isBooked → tappable, shows "● OPEN" or "⚡ CLOSING"
 *
 * This means future-date meals show all OPEN unless already booked
 * (in which case they show BOOKED), which is the correct behaviour
 * for both SELF and GUEST modes.
 * ─────────────────────────────────────────────────────────────────
 */
...
//
// For future dates (not today):
//   isOpen = true, isDeadline = true
//   → All meals are open for booking; hide cutoff text in UI.
//
// For today:
//   Normal cutoff math — isOpen depends on current time vs deadline.
//
// NOTE: isBooked is NOT computed here. It comes from the API and is
// merged in applyMealStatus separately so it always reflects the
// server's knowledge of existing bookings.
// ─────────────────────────────────────────────────────────────────
...
const getMealStatus = (meal, selectedDate = new Date()) => {
  ...
  // Future / past date → fully open, no cutoff text
  if (todayStr !== selStr) {
    return {
      isOpen: true,
      isOpen2: true,
      ...
      deadlineLabel: "",
      startLabel: "",
      isDeadline: true, // hides sub-text in MealTypePicker & MealCounter
    };
  }

  // Today → real cutoff math
...
// ── MealTypePicker ───────────────────────────────────────────────
// BOOKED vs CLOSED vs OPEN priority:
//   1. isBooked  → grey "✔ BOOKED" badge, disabled regardless of date
//   2. !isOpen   → grey "✕ CLOSED" badge, disabled (today only;
//                  future dates always have isOpen=true from getMealStatus)
//   3. tappable  → green "● OPEN" or amber "⚡ CLOSING"
//
// isDeadline=true (future date) means isOpen is always true from
// getMealStatus, so !isOpen is never true — only isBooked can disable.
// ────────────────────────────────────────────────────────────────
const MealTypePicker = ({ selected, onSelect, meals }) => (
  <View style={s.mealPickerGrid}>
    {meals.map((meal) => {
      ...
      // isBooked comes directly from the API (via applyMealStatus)
      const isBooked = !!meal.isBooked;

      // isOpen already accounts for date:
      //   future date → true (from getMealStatus isDeadline branch)
      //   today       → based on cutoff time
      const isOpen = meal.isOpen;

      // Disabled if booked OR if closed
      const isDisabled = isBooked || !isOpen;

      // Badge text + colour
      const badgeBg = isBooked
        ? "#9E9E9E"
        : !isOpen
          ? "#CCCCCC"
          : meal.isUrgent
      ...
      const badgeTxt = isBooked
        ...
        : !isOpen
          ...
          : meal.isUrgent
            ? "⚡  CLOSING"
            : "●  OPEN";
      // console.log(badgeTxt)
      // Band background
      const bandBg =
        isBooked || !isOpen ? "#F5F5F5" : meal.isUrgent ? "#FFF8E1" : meta.bg;

      // Name colour
      const nameColor = isBooked ? "#BBBBBB" : isOpen ? meta.color : "#CCCCCC";
      ...
      return (
        ...
        >
          <View
            style={[
              s.mealCard,
              isOpen && !isBooked && !meal.isUrgent && s.mealCardOpen,
              isOpen && !isBooked && meal.isUrgent && s.mealCardUrgent,
              (!isOpen || isBooked) && s.mealCardClosed,
              on && { borderColor: meta.color2, borderWidth: 2 },
            ]}
          >
            {/* Top colour band */}
            <View style={[s.mealCardBand, { backgroundColor: bandBg }]}>
              <Text style={[s.mealEmoji, isDisabled && s.mealEmojiDim]}>
                {meta.emoji}
              </Text>
              <View style={[s.statusBadge, { backgroundColor: badgeBg }]}>
                <Text style={s.statusBadgeTxt}>{badgeTxt}</Text>
              </View>
            </View>

            {/* Body */}
            <View style={s.mealCardBody}>
              <Text style={[s.mealCardName, { color: nameColor }]}>
                {meal.name}
              </Text>

              {badgeTxt === "●  OPEN" && <Text></Text>}
              {isBooked && <Text></Text>}
              {/*
               * Sub-text rules:
               *   isBooked     → show nothing (clean card)
               *   isDeadline   → future date, show nothing
               *   isUrgent     → countdown chip
               *   otherwise    → opens/closes line
               */}
              {!isBooked &&
                !meal.isDeadline &&
            ...
            {/* Dim veil for disabled cards */}
            {isDisabled && <View style={s.closedOverlay} />}
          </View>
...
const GuestDetailsForm = ({ vendorName, setVendorName, userData }) => (
  <Section title="Guest Details">
    ...
    {/* <FormInput
      ...
      placeholder="Enter purpose (e.g. Consultancy)"
      icon="📌"
    /> */}
    <FormInput
      label="Employee ID"
...
const ModeSelectCard = ({ mode, switchMode, setSelDate }) => (
...
const CreateOrderScreen = () => {
  ...
  const switchMode = (n) => {
    if (n !== mode) {
      fadeSwitch(() => {
        setMode(n);
        if (!isEdit) setSelDate(new Date()); // ✅ reset to today
      });
    }
  };
  ...
  const switchBookType = (n) => {
    if (n !== bookType) {
      fadeSwitch(() => {
        setBookType(n);
        if (!isEdit) setSelDate(new Date()); // ✅ reset
      });
    }
  };
  ...
  // ── Fetch meals whenever userData or selDate changes ─────────
  // The API receives userId + selDate so it can return:
  //   • cutoff_time / serviceStart_time  (for status calculation)
  //   • isBooked: true/false             (has the user booked this meal on selDate?)
  useEffect(() => {
    if (!userData || !selDate) return;
    fetchMeals(selDate);
  }, [userData, selDate]); // eslint-disable-line
  ...
  const fetchMeals = async (date) => {
    try {
      ...
      const res = await axios.post(
        `${API_URL}/api/meal`,
        { userId: userData?.user?.id, selDate: date },
        {
          headers: {
  ...
  // ── Merge API data + meta + date-aware status ────────────────
  // KEY RULE: isBooked is preserved verbatim from the API response.
  // getMealStatus only touches open/closed/urgent/countdown.
  // This means for future dates getMealStatus returns isOpen:true,
  // but isBooked can still be true → meal correctly shows as BOOKED.
  const applyMealStatus = (apiMeals, date) => {
    const merged = apiMeals.map((meal) => {
      ...
      if (bookType === "MONTHLY") {
        // ✅ FORCE all meals OPEN for monthly
        status = {
          isOpen: true,
          isOpen2: true,
          isToday: false,
          ...
          deadlineLabel: "",
          startLabel: "",
          isDeadline: true, // hides subtext
        };
      } else {
        // ✅ Normal logic for SINGLE
        status = getMealStatus(meal, date);
      }

      // ✅ NEW BOOKING LOGIC
      let isBookedFinal = false;

      if (mode === "SELF" && bookType === "MONTHLY") {
        isBookedFinal = !!meal.isSelfMonthlyBooked;
      } else if (mode === "SELF" && bookType === "SINGLE") {
        isBookedFinal = !!meal.isSelfSingleBooked;
      } else if (mode === "GUEST") {
        isBookedFinal = false; // 🔥 key requirement
      }
  ...
  // ── Reset selections on mode/bookType/date change (CREATE only)
  ...
  const buildPayload = () => {
    ...
    return {
      ...
      booking_type: "SINGLE",
      booking_for: mode,
      items: [{ meal_type_id: vendorMealId, quantity: 1 }],
    };
  };
                    ...
                    <Section title="Select Meal Type">
                      {loading ? (
                        <View style={s.loadWrap}>
                          ...
                          <Text style={s.loadTxt}>Loading meals…</Text>
                        </View>
                      ) : (
                        <MealTypePicker
